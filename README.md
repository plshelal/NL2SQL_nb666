# 金融问数系统 安装教程（请先通读一遍此教程，预先了解可能遇到的报错，如果你没有md格式阅读器，最好使用https://github.com/plshelal/readme来阅读）
...
3. **uv**（Python 包管理器）— 打开 PowerShell，粘贴下面一行回车：
   ```
     ...
     winget install astral-sh.uv（上面要是挂了就用这个备用链接，这个下载完之后需要重启powershell窗口使系统变量生效）
   ```
...
> - 🔌 **外部数据源**：查**行外数据**（LPR / CPI / PPI / 汇率 / 地产政策 / 财经资讯等同花顺 iFinD 数据）时，**先点亮它再提问**，此状态适用闲聊；只查行内指标（存贷款、利润、不良率等）不用开，默认直接走内部查询、最快，此状态不适用于闲聊。
>
